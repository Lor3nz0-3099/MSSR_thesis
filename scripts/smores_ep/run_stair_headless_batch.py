#!/usr/bin/env python3
"""Run reproducible Snake8 stair robustness episodes without the Isaac GUI."""
...
def parse_seed_expression(text: str) -> tuple[int, ...]:
    ...
    if not seeds:
        raise ValueError("At least one stair seed is required")
    if len(set(seeds)) != len(seeds):
        raise ValueError("Stair seeds must be unique")
    return tuple(seeds)
...
) -> tuple[tuple[str, UniformStairSpec], ...]:
    episodes: list[tuple[str, UniformStairSpec]] = []
    if include_reference:
        episodes.append(("reference", UniformStairSpec()))
    episodes.extend(
        (f"seed-{seed:06d}", sample_uniform_stair_spec(seed))
        for seed in seeds
    )
...
def evaluate_top_deck_result(
    payload: Mapping[str, Any],
    spec: UniformStairSpec,
    *,
    vertical_tolerance_m: float = 0.015,
) -> dict[str, Any]:
    ...
    required_center_z_m = (
        spec.top_heights_m[-1] + wheel_radius_m - vertical_tolerance_m
    )
    minimum_center_z_m = min(
    ...
    return {
        ...
        "required_minimum_center_z_m": required_center_z_m,
        "minimum_center_z_m": minimum_center_z_m,
        "all_modules_on_top_deck": (
            len(positions) == 8
            and connections == 7
            and minimum_center_z_m >= required_center_z_m
        ),
    }
...
def run_episode(
    ...
    if spec.seed is not None:
        launch_command.append(f"stair_seed:={spec.seed}")
    assembly_command = [
        "ros2",
        "run",
        "mssr_expert",
    ...
    try:
        ...
        assembly_log = (runtime_dir / "assembly.log").open("w", encoding="utf-8")
        ...
        # Allow one behavior-node graph callback after the stable topology.
        time.sleep(1.0)
        ...
        geometry = evaluate_top_deck_result(final_graph, spec)
        success = behavior.returncode == 0 and bool(
            geometry["all_modules_on_top_deck"]
        )
        result = {
            **manifest,
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPOSITORY_ROOT / "logs/stair_headless_batch",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for episode_id, spec in episodes:
        print(
            f"[{episode_id}] rise={spec.rise_m:.3f}m "
            f"depth={spec.tread_depth_m:.3f}m steps={spec.step_count}",
            flush=True,
        )
        ...
        if result.get("success") is False and not args.continue_on_failure:
            break
    summary = {
        "schema_version": "mssr.stair_headless_batch.v1",
