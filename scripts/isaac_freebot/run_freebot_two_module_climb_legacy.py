"""Phenomenological FreeBOT climb scenario with a magnet-following patch.

This diagnostic copy uses the compliant USD. It keeps the magnet attached to
its own shell through continuous compliant contact and applies the external
module attraction at the shell patch that follows the magnet. It is intentionally
separate from the physical and current scenario scripts.
"""
...
parser.add_argument(
    "--external-carrier-normal-scale",
    type=float,
    default=1.0,
    help=(
        "Legacy reduced-order coupling: also apply this fraction of the external "
        "normal attraction to the active carrier at the magnet position. The "
        "physical-contact baseline keeps this at zero."
    ),
)
...
parser.add_argument(
    ...
    default=True,
    help=(
        "Enable continuous compliant tangential contact between the internal "
        "magnet and its own active shell, with equal and opposite reactions."
    ),
)
parser.add_argument(
    "--adhesion-gap",
...
parser.add_argument("--dock-torsion", action=argparse.BooleanOptionalAction, default=False, help="Enable the old fixed-patch torsional hold; disabled in the mobile-patch baseline.")
parser.add_argument(
    "--external-model",
...
def active_to_passive_magnetic_force(shell_body, internal_body, magnet_frame):
    ...
    # The magnetized patch follows the carrier around the inside of the active
    # shell.  Target selection must therefore use this moving patch, not the
    # fixed line joining the two sphere centers.
    passive_distances = [
        max(np.linalg.norm(center - active_shell_point) - args.shell_radius, 0.0)
        for center in passive_centers
    ]
    ...
    patch_to_passive_center = passive_center - active_shell_point
    patch_center_distance = np.linalg.norm(patch_to_passive_center)
    if patch_center_distance < 1.0e-9:
        patch_to_passive_center_dir = center_line / center_distance
    else:
        patch_to_passive_center_dir = patch_to_passive_center / patch_center_distance
    passive_surface_point = passive_center - args.shell_radius * patch_to_passive_center_dir
    patch_delta = passive_surface_point - active_shell_point
    patch_gap = np.linalg.norm(patch_delta)
    if patch_gap < 1.0e-9:
        attraction_dir = center_line / center_distance
    else:
        attraction_dir = patch_delta / patch_gap
    shell_gap = max(center_distance - 2.0 * args.shell_radius, 0.0)
    axis_alignment = max(np.dot(radial_dir, attraction_dir), 0.0)
    capture = smoothstep((args.external_capture_gap - patch_gap) / max(args.external_capture_gap, 1.0e-9))

    # The attraction follows the moving magnetized patch.  Moving the carrier
    # away from the module-module contact increases patch_gap and decreases the
    # alignment, so attachment and detachment emerge continuously from geometry.
    decay_distance = max(args.external_decay_distance, 1.0e-9)
    alignment_factor = args.external_min_alignment + (
        1.0 - args.external_min_alignment
    ) * axis_alignment ** args.external_alignment_power
    force_norm = (
        args.external_max_force
        / (1.0 + patch_gap / decay_distance) ** 3
        * capture
        * alignment_factor
    )
    ...
    return force, active_shell_point, {
        ...
        "gap": patch_gap,
        "shell_gap": shell_gap,
        "shell_patch_gap": patch_gap,
        "magnetic_distance": patch_gap,
...
def internal_shell_adhesion_force(
    shell_body,
    internal_body,
    magnet_pos,
    internal_normal_force,
    contact_state,
    physics_dt,
):
    """Return compliant tangential contact between magnet and active shell.

    This contact exists independently of module-module docking. The spring
    integrates relative motion between the magnet and its current point on the
    moving shell, so it cannot anchor the mechanism to a fixed world point.
    """
    zero = np.zeros(3, dtype=np.float64)
    if not args.magnetic_adhesion:
        contact_state["displacement"] = zero.copy()
        contact_state["initialized"] = False
        return zero, zero, zero, 0.0, 0.0, False, "free"

    shell_pos = first_vec(shell_body.get_world_poses()[0])
    internal_pos = first_vec(internal_body.get_world_poses()[0])
    application_pos = np.asarray(magnet_pos, dtype=np.float64).reshape(-1, 3)[0]
    radial = application_pos - shell_pos
    radial_norm = np.linalg.norm(radial)
    normal_force_norm = np.linalg.norm(internal_normal_force)
    if radial_norm < 1.0e-9 or normal_force_norm < 1.0e-9:
        contact_state["displacement"] = zero.copy()
        contact_state["initialized"] = False
        return zero, application_pos, shell_pos, 0.0, 0.0, False, "free"

    normal_dir = radial / radial_norm
    shell_application_pos = shell_pos + args.inner_radius * normal_dir
    shell_lin, shell_ang = shell_body.get_velocities()
    internal_lin, internal_ang = internal_body.get_velocities()
    magnet_velocity = point_velocity(
        first_vec(internal_lin), first_vec(internal_ang), internal_pos, application_pos
    )
    shell_point_velocity = point_velocity(
        first_vec(shell_lin), first_vec(shell_ang), shell_pos, shell_application_pos
    )
    relative_velocity = magnet_velocity - shell_point_velocity
    tangential_velocity = relative_velocity - np.dot(relative_velocity, normal_dir) * normal_dir
    tangential_limit = min(
        args.adhesion_friction * normal_force_norm,
        args.adhesion_max_force,
    )

    if not contact_state.get("initialized", False):
        contact_state["initialized"] = True
        gravity_force = np.array(
            [0.0, 0.0, -args.adhesion_supported_mass * 9.81], dtype=np.float64
        )
        gravity_tangent = gravity_force - np.dot(gravity_force, normal_dir) * normal_dir
        desired_hold = clamp_vector(-gravity_tangent, tangential_limit)
    ...
    displacement = np.asarray(
        contact_state.get("displacement", zero), dtype=np.float64
    ).reshape(3)
...
def main():
    ...
    print("Running LEGACY PHENOMENOLOGICAL FreeBOT two-module climb test")
    ...
    print(
        ...
        f"legacy_carrier_scale={args.external_carrier_normal_scale}"
    )
    if args.external_model == "field-patch":
        print(
        ...
        print(
            ...
            "patch=magnet-following, alignment_gating=enabled"
        )
    print(
        "Materials: "
    ...
    print(
        "Internal magnet-shell contact: "
        f"enabled={args.magnetic_adhesion}, always_active=True, "
    ...
    print(
        "Dock diagnostic only: "
        f"align={args.dock_align_threshold}, patch={args.dock_patch_gap} m, "
        "no_drive_gating=True, geometric_detachment=True"
    )
    print(
        "Dock torsion: "
    ...
    for step in range(args.steps):
        ...
        external_metrics["dock_contact"] = dock_contact
        ...
        shell_body.apply_forces_and_torques_at_pos(
            ...
            positions=shell_force_pos.reshape(1, 3),
        )
        magnet_force_pos = np.asarray(magnet_pos, dtype=np.float64).reshape(-1, 3)[0]
        if abs(args.external_carrier_normal_scale) > 1.0e-12:
            # Historical phenomenological model: the unresolved magnetic load
            # was also applied directly to the carrier at the magnet. This is
            # intentionally opt-in because it is an equivalent coupling, not a
            # second independently measured physical force.
            internal_body.apply_forces_and_torques_at_pos(
                forces=(args.external_carrier_normal_scale * previous_external_force).reshape(1, 3),
                positions=magnet_force_pos.reshape(1, 3),
            )
        ...
        (
            ...
            adhesion_pos,
            adhesion_shell_pos,
            adhesion_tangent_force_norm,
            adhesion_tangent_limit,
            adhesion_active,
            adhesion_status,
        ) = internal_shell_adhesion_force(
            shell_body,
            internal_body,
            magnet_force_pos,
            internal_force,
        ...
        if adhesion_active:
            ...
            shell_center_for_adhesion = first_vec(shell_body.get_world_poses()[0])
            adhesion_normal = magnet_force_pos - shell_center_for_adhesion
            adhesion_normal /= max(np.linalg.norm(adhesion_normal), 1.0e-9)
            previous_adhesion_force -= (
                np.dot(previous_adhesion_force, adhesion_normal) * adhesion_normal
            )
        else:
            previous_adhesion_force = np.zeros(3, dtype=np.float64)
        if adhesion_active and np.linalg.norm(previous_adhesion_force) > 1.0e-6:
            ...
            shell_body.apply_forces_and_torques_at_pos(
                forces=(-previous_adhesion_force).reshape(1, 3),
                positions=np.asarray(adhesion_shell_pos, dtype=np.float64).reshape(1, 3),
            )

        dock_torsion_torque = np.zeros(3, dtype=np.float64)
        attraction_value = external_metrics.get("attraction_dir")
        if args.dock_torsion and dock_contact and attraction_value is not None:
        ...
        if args.log_interval > 0 and step % args.log_interval == 0:
            ...
            adhesion_torque = np.asarray(
                np.cross(np.asarray(adhesion_pos) - internal_pos, previous_adhesion_force),
                dtype=np.float64,
            ).reshape(-1, 3)[0]
            print(
                ...
                f"contact={'1' if dock_contact else '0'}  "
                "patch_mobile=1  "
